...
export default function () {
  it('Changing active cell should fire an event', function (done) {
    ...
    try {
      doAssert(
        previousActiveCell.columnIndex === 0,
        'Expected proper columnIndex before move.',
      );
      doAssert(
        previousActiveCell.rowIndex === 0,
        'Expected proper rowIndex before move.',
      );
      ...
      doAssert(
        grid.activeCell.columnIndex === 2,
        'Expected proper columnIndex after move.',
      );
      doAssert(
        grid.activeCell.rowIndex === 0,
        'Expected proper rowIndex after move.',
      );
    } catch (error) {
      return done(error);
    }
