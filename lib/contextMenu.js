...
export default function (self) {
   ...
   * @returns {[any,any]} column schemas tuple (each schema has an additional field `schemaIndex`)
   */
  ...
  function addDefaultContextMenuItem(e) {
      ...
      if (e.cell && e.cell.header && e.cell.columnIndex > -1) {
        ...
        e.items.push({
          ...
          click: function (ev) {
            ...
            if (contiguousColumns) {
              const orders = self.orders.columns;
              const from = contiguousColumns[0].orderIndex;
              const to = contiguousColumns[1].orderIndex;
              for (let orderIndex = from; orderIndex <= to; orderIndex++) {
    ...
    //#region unhide rows
    const canUnhideRows = e.cell.isRowHeader && e.cell.header;
    if (canUnhideRows) {
    }
    //#endregion unhide rows
