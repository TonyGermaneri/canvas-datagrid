...
export default function (self) {
  ...
  function drawCellTreeIcon(cell, tree, rowTree) {
    const pc = rowTree ? tree.parentCount : 0;
    const rc = self.style.cellTreeIconWidth * self.scale,
      mt = self.style.cellTreeIconMarginTop * self.scale,
      mr = self.style.cellTreeIconMarginRight * self.scale,
      ml =
        self.style.cellTreeIconMarginLeft * self.scale +
        pc * (rc + cell.paddingLeft);

    let x = cell.x + cell.paddingLeft + self.canvasOffsetLeft + ml,
      y = cell.y + self.canvasOffsetTop + mt;
    ...
    if (tree.icon) {
      ...
      self.ctx.fillRect(x, y, rc, rc);
      self.ctx.strokeStyle = self.style.cellTreeIconBorderColor;
      self.ctx.rect(x, y, rc, rc);
      self.ctx.stroke();

      self.ctx.beginPath();
      if (tree.expand) {
        self.ctx.moveTo(x + 3, y + rc * 0.5);
        self.ctx.lineTo(x + rc - 3, y + rc * 0.5);
      } else {
        self.ctx.moveTo(x + 2, y + rc * 0.5);
        self.ctx.lineTo(x + rc - 2, y + rc * 0.5);
        self.ctx.moveTo(x + rc * 0.5, y + 2);
        self.ctx.lineTo(x + rc * 0.5, y + rc - 2);
      }
    ...
    function drawCell(rowData, rowOrderIndex, rowIndex) {
      return function drawEach(header, headerIndex, columnOrderIndex) {
        ...
        cell = {
          ...
          isRowTree:
            rowOrderIndex >= 0 &&
            columnOrderIndex == self.celltree.rowsColIndex &&
            ...
            if (!self.dispatchEvent('rendertext', ev)) {
              if (cell.innerHTML || header.type === 'html') {
                ...
                let treeCellPadding = 0,
                  isDrawText = true;
                if (
                  cell.columnIndex == self.celltree.rowsColIndex &&
                  !cell.isColumnHeader &&
                  self.celltree.rows.length > 0 &&
                  Object.keys(self.celltree.rows[cell.rowIndex]).length > 0
                )
