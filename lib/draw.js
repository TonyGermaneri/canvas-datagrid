...
define([], function () {
    ...
    return function (self) {
        self.drawOrderByArrow = function (x, y) {
        ...
        self.drawTreeArrow = function (cell, x, y) {
            x += self.canvasOffsetLeft;
            y += self.canvasOffsetTop;
        ...
        self.radiusRect = function (x, y, w, h, radius) {
            x += self.canvasOffsetLeft;
            y += self.canvasOffsetTop;
            var r = x + w, b = y + h;
            self.ctx.beginPath();
        ...
        self.addBorderLine = function (c, pos) {
            self.ctx.beginPath();
            var p = {
                t: function () {
        ...
        self.draw = function (internal) {
            ...
            function drawScrollBars() {
                ...
                self.fillRect(v.x, v.y, v.width, v.height + m);
                self.strokeRect(v.x, v.y, v.width, v.height + m);
                // vertical box
                ...
                if (vb.width < v.width) {
                    self.radiusRect(vb.x, vb.y, vb.width, vb.height, self.style.scrollBarBoxBorderRadius);
                    self.ctx.stroke();
                    self.ctx.fill();
                }
                // horizontal
                ...
                self.ctx.fillStyle = self.style.scrollBarBackgroundColor;
                self.fillRect(n.x, n.y, n.width + m, n.height);
                self.strokeRect(n.x, n.y, n.width + m, n.height);
                // horizontal box
                ...
                if (nb.width < n.width) {
                    self.radiusRect(nb.x, nb.y, nb.width, nb.height, self.style.scrollBarBoxBorderRadius);
                    self.ctx.stroke();
                    self.ctx.fill();
                }
                //corner
                ...
                self.radiusRect(co.x, co.y, co.width, co.height, 0);
                self.ctx.stroke();
                self.ctx.fill();
            ...
            function drawCell(d, rowIndex, rowOrderIndex) {
                return function drawEach(header, headerIndex, columnOrderIndex) {
                    ...
                    if (!self.dispatchEvent('beforerendercell', [{}, self.ctx, d[header.name], d, header, cx, cy], self.intf)) {
                        ...
                        if (!cell.isGrid) {
                            self.fillRect(cx, cy, cell.width, cell.height);
                            self.strokeRect(cx, cy, cell.width, cell.height);
                        }
                        self.ctx.save();
                        self.radiusRect(cell.x, cell.y, cell.width, cell.height, 0);
                        self.ctx.clip();
                        ...
                        if (cellStyle === 'rowHeaderCell' && self.attributes.tree) {
                            if (!self.dispatchEvent('rendertreearrow', [{}, self.ctx, cell], self.intf)) {
                                treeArrowSize = self.drawTreeArrow(cell, self.style[cellStyle + 'PaddingLeft'], cy, 0);
                            }
                        }
                            ...
                            if (cell.isGrid) {
                                ...
                                if (cellStyle === 'headerCell' && self.orderBy === header.name) {
                                    if (!self.dispatchEvent('renderorderbyarrow', [{}, self.ctx, cell], self.intf)) {
                                        orderByArrowSize = self.drawOrderByArrow(cx + self.style[cellStyle + 'PaddingLeft'], 0);
                                    }
                                }
                                ...
                                self.fillText(self.addEllipsis(cell.formattedValue, cell.width
                        ...
                        if (active && !self.attributes.rowSelectionMode) {
                            ...
                            self.strokeRect(aCell.x, aCell.y, aCell.width, aCell.height);
                        }
                        if (selected) {
            ...
            function drawRow(r, d) {
                ...
                if (self.attributes.rowSelectionMode
                    ...
                    self.ctx.lineWidth = self.style.activeCellOverlayBorderWidth;
                    self.ctx.strokeStyle = self.style.activeCellOverlayBorderColor;
                    self.strokeRect(0, y, self.getHeaderWidth(), rowHeight);
                }
            ...
            function drawReorderMarkers() {
                ...
                if (self.dragMode === 'row-reorder') {
                    ...
                    self.fillRect(b.x, b.y, b.width, b.height);
                    self.strokeRect(b.x, b.y, b.width, b.height);
                    self.ctx.lineWidth = self.style.reorderMarkerIndexBorderWidth;
                    self.ctx.strokeStyle = self.style.reorderMarkerIndexBorderColor;
                    if (self.currentCell.rowIndex !== self.reorderObject.rowIndex
                            && self.currentCell.rowIndex - 1 !== self.reorderObject.rowIndex) {
                        self.addBorderLine(m, 't');
                    }
            ...
            function drawBorder() {
                ...
                self.strokeRect(0, 0, self.width, self.height);
            }
            function drawDebug() {
                if (self.attributes.showPerformance) {
                ...
                if (self.attributes.debug) {
                    ...
                    p.split('\n').forEach(function (s, index) {
                        ...
                        self.fillRect(70, 39 + (index * 15), self.ctx.measureText(s).width + 20, 15);
                        self.ctx.fillStyle = 'rgba(0, 0, 0, 1)';
                        self.fillText(s, 80, 52 + (index * 15));
                    });
                }
            }
            self.ctx.save();
