...
export default function (self) {
  ...
  self.draw = function (internal) {
    ...
    function drawRow(rowOrderIndex, rowIndex) {
      ...
      if (rowOrderIndex !== viewData.length && rowOpen) {
        ...
        if (self.intf.parentNode) {
          const { columnGroupsAreaHeight, rowGroupsAreaWidth } = self.intf.parentNode;
    ...
    function drawGroupArea() {
      ...
      const baseIndex = self.scrollIndexLeft
        /** @see function `drawHeaderColumnRange` in function `drawHeaders` */
      let _baseX =
        -self.scrollBox.scrollLeft +
        self.scrollPixelLeft +
        self.style.columnHeaderCellBorderWidth;
      if (self.attributes.showRowHeaders)
        _baseX += rowHeaderCellWidth;
      _baseX += mx;

      let baseY = self.style.columnHeaderCellHeight * 0.25;
      baseY -= my;

      const toggleHandleWidth = self.style.columnHeaderCellHeight * 0.5;
      _baseX -= toggleHandleWidth * 1.5

      /**
       * Index is columnIndex, value is baseX
       * @type {number[]}
       */
      let baseXs;
      if (self.groupedColumns.length > 0) {
        const schema = self.getSchema();
        baseXs = new Array(schema.length + 1);
        const getColumnWidth = (index) => {
          const width = self.sizes.columns[index];
          if (width) return width;
          const s = schema[index];
          if (!s || s.hidden) return 0;
          if (s.width >= 0) return s.width;
          return self.style.cellWidth;
        };

        let prevBaseX = _baseX;
        baseXs[baseIndex] = _baseX;
        for (let i = baseIndex - 1; i >= 0; i--) {
          prevBaseX -= getColumnWidth(i);
          baseXs[i] = prevBaseX;
        }
        prevBaseX = _baseX;
        for (let i = baseIndex + 1; i <= schema.length; i++) {
          prevBaseX += getColumnWidth(i - 1);
          baseXs[i] = prevBaseX;
        }
        if (baseXs[0] === prevBaseX && prevBaseX < 0) {
          // when a user creates a group that contains all columns and user collapse it
          const v = mx + rowHeaderCellWidth - toggleHandleWidth * 1.5;
          baseXs.fill(v);
        }
      ...
      for (let row = 0; row < self.groupedColumns.length; row++) {
        ...
        for (let j = 0; j < groups.length; j++) {
          const { from, to, collapsed } = groups[j];
          ctx.save();
          ctx.strokeStyle = self.style.grounpIndicatorColor;
          ctx.fillStyle = self.style.grounpIndicatorBackgroundColor;
          const baseX = baseXs[from];
          const nextX = baseXs[to + 1];
          if (nextX < 0) continue; // invisble
          drawGroupHandle(baseX, baseY, toggleHandleWidth, collapsed);
          self.visibleGroups.push({
            type: 'c',
            x: baseX + mx, y: baseY + my,
            x2: Math.max(nextX, baseX + toggleHandleWidth) + mx,
            y2: baseY + toggleHandleWidth + my,
            collapsed, from, to, row,
          });
          if (!collapsed && nextX > baseX) {
            ctx.beginPath();
            ctx.moveTo(baseX + toggleHandleWidth, baseY + toggleHandleWidth * 0.5);
            ctx.lineTo(nextX, baseY + toggleHandleWidth * 0.5);
            ctx.lineTo(nextX, baseY + toggleHandleWidth);
            ctx.stroke();
          }
          ctx.restore();
        }
        baseY += self.style.columnHeaderCellHeight;
      }
    }
