  ...
  self.draw = function (internal) {
    ...
    function drawRowHeader(rowData, rowIndex, rowOrderIndex) {
      if (self.attributes.showRowHeaders) {
        ...
        const rowHeaderValue =
          self.hasActiveFilters() || self.hasCollpasedRowGroup()
            ? filteredRowNumber
            : rowIndex + 1;
    ...
    function drawGroupArea() {
      ...
      if (groupedColumns.length > 0) {
        ...
        const colIndexes = Object.keys(columnGroupsRectInfo).sort(
          (a, b) => a - b,
        );
        const maxColIndex = parseInt(colIndexes[colIndexes.length - 1], 10);

        for (let i = 0; i < colIndexes.length; i++) {
          const colIndex = parseInt(colIndexes[i], 10);
          for (let j = 0; j < groupedColumns.length; j++) {
            const group = groupedColumns[j];
            let leftmost = group.from;
            let rightmost = group.to;
            let inGroup;

            if (onTheLeft) {
              leftmost--;
              inGroup = group.collapsed
                ? colIndex === leftmost
                : colIndex >= leftmost && colIndex <= rightmost;
            } else {
              rightmost++;
              if (rightmost >= schema.length) rightmost = maxColIndex;
            ...
            const topY = -my + group.row * rowHeight;
            const centerY = topY + rowHeight * 0.5;

            const { x, w: colWidth } = columnGroupsRectInfo[colIndex];
            const drawLine = (isEnd) => {
              const leftX = Math.max(minX4line, x);
              const rightX = Math.max(minX4line, x + colWidth);
              const x0 = onTheLeft ? leftX : rightX;
              const x1 = onTheLeft ? rightX : leftX;
              const coords = [x0, centerY, x1, centerY];
              if (isEnd) coords.push(x1, centerY + toggleHandlePadding);
              strokeLines(coords);
            };

            let minX4line = rowHeaderCellWidth;
            if (group.from > 0) minX4line += frozenColumnsWidth;

            ctx.save();
            ctx.strokeStyle = self.style.grounpIndicatorColor;
            ctx.fillStyle = self.style.grounpIndicatorBackgroundColor;
            if (onTheLeft) {
              // The indicator on the left
              const x1 = x + colWidth - toggleHandlePadding - toggleHandleSize;
              const x2 = x1 + toggleHandlePadding;
              if (colIndex === leftmost) {
                // Leftmost cell of this group
                if (colIndex === -1 || x1 >= minX4line) {
                  if (!group.collapsed)
                    strokeLines([x2, centerY, x + colWidth, centerY]);
                  drawGroupHandle(
                    x1,
              ...
              } else if (colIndex === rightmost) {
                // Rightmost cell of this group
                drawLine(true);
              } else {
                drawLine(false);
              }
              if (typeof group.x1 !== 'number' || x1 < group.x1) group.x1 = x1;
              if (typeof group.x2 !== 'number' || x2 > group.x2) group.x2 = x2;
            } else {
              // The indicator on the right
              let x2 =
                rightmost === group.to + 1
                  ? x + toggleHandlePadding
                  : x + colWidth - toggleHandlePadding - toggleHandleSize;
              if (colIndex === leftmost) {
                // Leftmost cell of this group
                drawLine(true);
              } else if (colIndex === rightmost) {
                if (!group.collapsed) strokeLines([x, centerY, x2, centerY]);
                drawGroupHandle(
                  x2,
                  topY + toggleHandlePadding,
                  toggleHandleSize,
              ...
              x2 = x2 + toggleHandleSize;
              if (typeof group.x1 !== 'number' || x < group.x1) group.x1 = x;
              if (typeof group.x2 !== 'number' || x2 > group.x2) group.x2 = x2;
            }
            ctx.restore();
          }
        ...
        for (let i = 0; i < groupedColumns.length; i++) {
          const { row, x1, x2, collapsed, from, to } = groupedColumns[i];
          const topY = -my + row * rowHeight;
          self.visibleGroups.push({
            type: 'c',
            x: x1 + mx,
            y: topY + my,
            x2: Math.max(x2, x1 + toggleHandleSize + toggleHandlePadding) + mx,
            y2: topY + rowHeight + my,
            collapsed,
            from,
            to,
            row,
      ...
      if (groupedRows.length > 0) {
        const columnWidth = self.style.rowGroupColumnWidth;
        const toggleHandleSize = columnWidth * 0.5;
        const toggleHandlePadding = (columnWidth - toggleHandleSize) * 0.5;
        const minY4line = columnHeaderCellHeight;

        const rowIndexes = Object.keys(rowGroupsRectInfo).sort((a, b) => a - b);
        for (let i = 0; i < rowIndexes.length; i++) {
          const rowIndex = parseInt(rowIndexes[i], 10);
          for (let j = 0; j < groupedRows.length; j++) {
            const group = groupedRows[j];
            let topmost = group.from;
            let bottommost = group.to;
            let inGroup;
            let moveUpForLastRow = false;

            if (onTheTop) {
              topmost--;
              inGroup = group.collapsed
                ? rowIndex === topmost
                : rowIndex >= topmost && rowIndex <= bottommost;
            } else {
              bottommost++;

              const lastRow = viewData.length - 1;
              moveUpForLastRow = lastRow >= 0 && bottommost > lastRow;
              if (moveUpForLastRow) bottommost = lastRow;
            ...
            let { y, h: rowHeight } = rowGroupsRectInfo[rowIndex];
            const leftX = -mx + group.col * columnWidth;
            const centerX = leftX + columnWidth * 0.5;

            const drawLine = (isEnd) => {
              let topY = Math.max(minY4line, y);
              const bottomY = Math.max(minY4line, y + rowHeight);
              if (
                !group.paddedFrozenMark &&
                rowGroupsFrozenInfo.y &&
                y > rowGroupsFrozenInfo.y
              ) {
                topY = Math.max(minY4line, topY - rowGroupsFrozenInfo.h + 0.5);
                group.paddedFrozenMark = true;
              }
              const y0 = onTheTop ? topY : bottomY;
              const y1 = onTheTop ? bottomY : topY;
              const coords = [centerX, y0, centerX, y1];
              if (isEnd) coords.push(centerX + toggleHandlePadding, y1);
              strokeLines(coords);
            };
            ...
            if (onTheTop) {
              ...
              const y1 = y + rowHeight - toggleHandlePadding - toggleHandleSize;
              const y2 = y1 + toggleHandlePadding;
              if (rowIndex === group.from - 1) {
                if (rowIndex === -1 || y1 >= minY4line) {
                  if (!group.collapsed)
                    strokeLines([centerX, y2, centerX, y + rowHeight]);
                  drawGroupHandle(
              ...
              } else if (rowIndex === group.to) {
                drawLine(true);
              } else {
                drawLine(false);
              }
              if (typeof group.y1 !== 'number' || y1 < group.y1) group.y1 = y1;
              if (typeof group.y2 !== 'number' || y2 > group.y2) group.y2 = y2;
            } else {
              // The indicator on the bottom
              if (rowIndex === topmost) {
                // Leftmost cell of this group
                drawLine(true);
              } else if (rowIndex === bottommost) {
                if (y >= minY4line) {
                  if (moveUpForLastRow) {
                    if (!group.collapsed) drawLine(false);
                    y += rowHeight - toggleHandleSize;
                  }
                  drawGroupHandle(
                    leftX + toggleHandlePadding,
                    y,
              ...
              const y2 = y + toggleHandleSize;
              if (typeof group.y1 !== 'number' || y < group.y1) group.y1 = y;
              if (typeof group.y2 !== 'number' || y2 > group.y2) group.y2 = y2;
            }
            ctx.restore();
          }
          for (let i = 0; i < groupedRows.length; i++) {
            const { col, y1, y2, collapsed, from, to } = groupedRows[i];
            const leftX = -mx + col * columnWidth;
            self.visibleGroups.push({
              type: 'r',
              x: leftX + mx,
              y: y1 + my,
              x2: leftX + columnWidth + mx,
              y2:
                Math.max(y2, y1 + toggleHandleSize + toggleHandlePadding) + my,
              collapsed,
              from,
              to,
              col,
