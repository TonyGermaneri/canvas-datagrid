...
export default function (self) {
  ...
  function drawGroupHandle(x, y, width, collapsed) {
    ...
    ctx.moveTo(x + width * 0.2, cy);
    ctx.lineTo(x + width * 0.78, cy);
    ctx.stroke();
    if (collapsed) {
      ctx.beginPath();
      ctx.moveTo(cx, y + width * 0.22);
      ctx.lineTo(cx, y + width * 0.8);
      ctx.stroke();
    }
  }
  ...
  self.draw = function (internal) {
    ...
    var checkScrollHeight,
      ...
      collpasedColumnGroups = self.getCollpasedColumnGroups(),
      collpasedRowGroups = self.getCollpasedRowGroups(),
    ...
    /**
     * @param {number} boundRowIndex
     * @param {number} y Y of top border
     * @param {number} h
     */
    function saveRowGroupsRectInfo(boundRowIndex, y, h) {
      if (rowGroupsRectInfo[boundRowIndex]) return;
      rowGroupsRectInfo[boundRowIndex] = { y, h };
    }
    /**
     * @param {number} columnIndex
     * @param {number} x X of left border
     * @param {number} w
     */
    function saveColumnGroupsRectInfo(columnIndex, x, w) {
      if (columnGroupsRectInfo[columnIndex]) return;
      columnGroupsRectInfo[columnIndex] = { x, w };
    }
    /**
     * @param {number} columnIndex
     * @returns {boolean}
    ...
    function isColumnCollapsedByGroup(columnIndex) {
      return (
        collpasedColumnGroups.findIndex(
    ...
    function drawCell(rowData, rowOrderIndex, rowIndex) {
      return function drawEach(header, headerIndex, columnOrderIndex) {
        ...
        saveRowGroupsRectInfo(cell.boundRowIndex, cell.y, cell.height);
        saveColumnGroupsRectInfo(cell.columnIndex, cell.x, cell.width);
        ...
        if (
          ...
          if (hasRowGap && collpasedRowGroups.length > 0) {
            hasRowGap =
              collpasedRowGroups.find(
                (group) =>
      ...
      if (groupedColumns.length > 0) {
        ...
        for (let i = 0; i < colIndexes.length; i++) {
          ...
          for (let j = 0; j < groupedColumns.length; j++) {
            ...
            const inGroup = colIndex >= group.from - 1 && colIndex <= group.to;
            if (!inGroup) continue;
            ...
            const drawLine = (isEnd) => {
              const drawX = Math.max(minX4line, x);
              const drawX2 = Math.max(minX4line, x + colWidth);
              ctx.beginPath();
              ctx.moveTo(drawX, centerY);
              ctx.lineTo(drawX2, centerY);
              if (isEnd) ctx.lineTo(drawX2, centerY + toggleHandlePadding);
              ctx.stroke();
            };
            ...
            if (colIndex === String(group.from - 1)) {
              if (colIndex === '-1' || x1 >= minX4line) {
                if (!group.collapsed) {
                  ctx.beginPath();
                  ctx.moveTo(x2, centerY);
                  ctx.lineTo(x + colWidth, centerY);
                  ctx.stroke();
                }
                drawGroupHandle(
                  x1,
            ...
            } else if (colIndex === String(group.to)) {
              drawLine(true);
            } else {
              drawLine(false);
            }
      ...
      if (groupedRows.length > 0) {
        ...
        for (let i = 0; i < rowIndexes.length; i++) {
          ...
          for (let j = 0; j < groupedRows.length; j++) {
            ...
            const inGroup = rowIndex >= group.from - 1 && rowIndex <= group.to;
            if (!inGroup) continue;
            ...
            const drawLine = (isEnd) => {
              let drawY = Math.max(minY4line, y);
              const drawY2 = Math.max(minY4line, y + rowHeight);
              if (
                !group.paddedFrozenMark &&
                rowGroupsFrozenInfo.y &&
                y > rowGroupsFrozenInfo.y
              ) {
                drawY -= rowGroupsFrozenInfo.h + 0.5;
                group.paddedFrozenMark = true;
              }

              ctx.beginPath();
              ctx.moveTo(centerX, drawY);
              ctx.lineTo(centerX, drawY2);
              // console.log(rowIndex, y, rowGroupsFrozenInfo);
              if (isEnd) ctx.lineTo(centerX + toggleHandlePadding, drawY2);
              ctx.stroke();
            };
            ...
            ctx.strokeStyle = self.style.grounpIndicatorColor;
            ctx.fillStyle = self.style.grounpIndicatorBackgroundColor;
            if (rowIndex === String(group.from - 1)) {
              if (rowIndex === '-1' || y1 >= minY4line) {
                if (!group.collapsed) {
                  ctx.beginPath();
                  ctx.moveTo(centerX, y2);
                  ctx.lineTo(centerX, y + rowHeight);
                  ctx.stroke();
                }
                drawGroupHandle(
            ...
            } else if (rowIndex === String(group.to)) {
              drawLine(true);
            } else {
              drawLine(false);
            }
