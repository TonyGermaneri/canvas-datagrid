...
export default function (self) {
  ...
  self.draw = function (internal) {
    ...
    function drawGroupArea() {
      ...
      const baseXs = [_baseX - toggleHandleWidth * 1.5];
      if (self.groupedColumns.length > 0) {
        const schema = self.getSchema();
        let prevBaseX = baseXs[0];
        for (let i = 0; i < schema.length; i++) {
          const s = schema[i];
          let width = 0;
          if(!s.hidden)
            width = self.sizes.columns[i] || self.style.cellWidthWithChildGrid;
          prevBaseX += width;
          baseXs.push(prevBaseX);
        }
      }
