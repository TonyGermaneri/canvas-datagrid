        ...
        self.draw = function (internal) {
            ...
            function drawDebug() {
                if (self.attributes.debug) {
                    ...
                    var d = {
                            perf: (performance.now() - p).toFixed(2) + 'ms',
                            scrollLeft: self.scrollBox.scrollLeft,
                            scrollTop: self.scrollBox.scrollTop,
                            scrollIndexTop: self.scrollIndexTop,
                            scrollPixelTop: self.scrollPixelTop,
                            scrollIndexLeft: self.scrollIndexLeft,
                            scrollPixelLeft: self.scrollPixelLeft,
                            canvasOffsetLeft: self.canvasOffsetLeft,
                            canvasOffsetTop: self.canvasOffsetTop,
                            width: self.width,
                            height: self.height,
                            mouse: self.mouse,
                            entities: self.visibleCells.length,
                    ...
                    Object.keys(d).forEach(function (key, index) {
                        var m = key + ': ' + d[key], h = 17;
                        self.ctx.fillStyle = 'rgba(0, 0, 0, 1)';
                        fillRect(70, 39 + (index * h), self.ctx.measureText(m).width + 20, h);
                        self.ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
                        fillText(m, 80, 52 + (index * h));
                    });
                }
            }
            self.ctx.save();
