    ...
    function grid(args) {
        ...
        function setScrollArea() {
            ...
            scrollBox.scrollHeight = scrollHeight - headerCellBorder
                + style.scrollDivOverlap - container.offsetHeight + getHeaderCellHeight() + style.scrollBarWidth;
            scrollBox.scrollWidth = scrollWidth - style.scrollBarWidth
                - container.offsetWidth + getHeaderCellWidth() + (style.scrollBarWidth * 2);
            scrollBox.scrollBarHeight = 15;
            scrollBox.scrollBarWidth = 15;
        ...
        function draw() {
            ...
            var checkScrollHeight, borderWidth, rowHeaderCell, p, cx, cy, treeGrid, rowOpen,
                rowHeight, cornerCell, y, x, c, h, w, s, r, rd, l = data.length, scrollLeft = 0,
            ...
            function drawScrollBars() {
                ...
                // vertical
                v.x = w - style.scrollBarWidth - style.scrollBarBoxMargin;
                v.y = headerCellHeight;
                v.w = style.scrollBarWidth + m;
                v.h = h - headerCellHeight - style.scrollBarWidth - m;
                ctx.fillStyle = style.scrollBarBackgroundColor;
                ctx.fillRect(v.x, v.y, v.w, v.h + m);
                ctx.strokeRect(v.x, v.y, v.w, v.h + m);
                // vertical box
                vb.x = v.x + style.scrollBarBoxMargin;
                vb.y = headerCellHeight + style.scrollBarBoxMargin
                    + ((v.h - scrollBox.scrollBoxHeight)
                        * (scrollBox.scrollTop / scrollBox.scrollHeight));
                vb.w = style.scrollBarBoxWidth;
                vb.h = scrollBox.scrollBoxHeight;
                ctx.fillStyle = style.scrollBarBoxColor;
                if (/vertical/.test(c.context)) {
                    ctx.fillStyle = style.scrollBarActiveColor;
                }
                radiusRect(vb.x, vb.y, vb.w, vb.h, style.scrollBarBoxBorderRadius);
                ctx.stroke();
                ctx.fill();
                // horizontal
                n.x = headerCellWidth;
                n.y = h - style.scrollBarWidth - style.scrollBarBoxMargin;
                n.w = w - style.scrollBarWidth - headerCellWidth - m;
                n.h = style.scrollBarWidth + style.scrollBarBoxMargin;
                ctx.fillStyle = style.scrollBarBackgroundColor;
                ctx.strokeStyle = style.scrollBarBorderColor;
                ctx.lineWidth = style.scrollBarBorderWidth;
                ctx.fillRect(n.x, n.y, n.w + m, n.h);
                ctx.strokeRect(n.x, n.y, n.w + m, n.h);
                // horizontal box
                nb.y = n.y + style.scrollBarBoxMargin;
                nb.x = headerCellWidth + style.scrollBarBoxMargin
                    + ((n.w - scrollBox.scrollBoxWidth)
                        * (scrollBox.scrollLeft / scrollBox.scrollWidth));
                nb.w = scrollBox.scrollBoxWidth;
                nb.h = style.scrollBarBoxWidth;
                ctx.fillStyle = style.scrollBarBoxColor;
                if (/horizontal/.test(c.context)) {
                    ctx.fillStyle = style.scrollBarActiveColor;
                }
                radiusRect(nb.x, nb.y, nb.w, nb.h, style.scrollBarBoxBorderRadius);
                ctx.stroke();
                ctx.fill();
                scrollBox.bar = {
                    v: v,
            ...
            function drawRow(r) {
                if ((y - cellHeight > h) || (y < cellHeight * -1)) {
                    return false;
                }
                rd = data[r];
        ...
        function resize() {
            ...
            scrollBox.width = container.offsetWidth - headerCellWidth;
            scrollBox.height = container.offsetHeight - headerCellHeight - headerCellBorder
                + style.scrollDivOverlap;
        ...
        function getCellAt(x, y) {
            var i, l = visibleCells.length, cell, cntx;
            if (x > container.offsetWidth - style.scrollBarWidth &&
                    y > getHeaderCellHeight()) {
                // vertical scrolling
                cntx = 'vertical-scroll-box';
                if (y > scrollBox.box.v.y + scrollBox.scrollBoxHeight) {
                    cntx = 'vertical-scroll-bottom';
                } else if (y < scrollBox.box.v.y) {
                    cntx = 'vertical-scroll-top';
                }
            ...
            if (y > container.offsetHeight - style.scrollBarWidth &&
                    x > getHeaderCellWidth()) {
                // horizontal scrolling
                cntx = 'horizontal-scroll-box';
                if (x > scrollBox.box.h.x + scrollBox.scrollBoxHeight) {
                    cntx = 'horizontal-scroll-right';
                } else if (x < scrollBox.box.h.x) {
                    cntx = 'horizontal-scroll-left';
                }
                container.style.cursor = 'default';
                return {
                    context: cntx
        ...
        function mousemove(e) {
            ...
            if (!resizingItem && o) {
                ...
                if (selecting
                    ...
                    if (x > container.offsetWidth - attributes.selectionScrollZone) {
                        scrollBox.scrollLeft += attributes.selectionScrollIncrement;
                        setTimer = true;
                    }
                    if (y > container.offsetHeight - attributes.selectionScrollZone) {
        ...
        function scrollGrid(e) {
            var pos = {
                ...
                vOffset = style.scrollBarBoxWidth + getHeaderCellHeight() + scrollBox.scrollBoxHeight,
                hOffset = style.scrollBarBoxWidth + getHeaderCellWidth() + scrollBox.scrollBoxWidth;
            scrollMode = currentCell.context;
            ...
            if (scrollMode === 'vertical-scroll-box') {
                scrollBox.scrollTop = scrollBox.scrollHeight *
                    ((pos.y + scrollBox.scrollBoxHeight - vOffset)
                        / (container.offsetHeight - vOffset));
            ...
            if (scrollMode === 'horizontal-scroll-box') {
                scrollBox.scrollLeft = scrollBox.scrollWidth *
                    ((pos.x + scrollBox.scrollBoxHeight - hOffset)
                        / (container.offsetWidth - hOffset));
        ...
        function mousedown(e) {
            ...
            if (scrollModes.indexOf(resizeMode) !== -1) {
                scrollMode = resizeMode;
                scrollStartMode = scrollMode;
                scrollGrid(e);
        ...
        function initScrollBox() {
            var evs = {},
                ...
                scrollBoxHeight = 20,
                scrollBoxWidth = 20;
            scrollBox.addEventListener = function (eventName, fn) {
