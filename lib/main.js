 ...
 */
    function grid(args) {
        ...
        function draw(internal) {
            ...
            function drawCell(d, rowIndex) {
                return function drawEach(header, headerIndex) {
                    var cellStyle = header.style || 'cell',
                        ...
                        active = activeCell.rowIndex === rowIndex && activeCell.columnIndex === headerIndex,
                        isGrid = Array.isArray(d[header.name]),
                        activeHeader = (activeCell.rowIndex === rowIndex || activeCell.columnIndex === headerIndex)
                            && (headerIndex === -1 || rowIndex === -1),
                        val,
            ...
            function drawRows() {
                ...
                if (attributes.showNewRow) {
                    ...
                    for (o = scrollIndexLeft; o < g; o += 1) {
                        i = columnOrder[o];
                        x += drawCell(newRow, data.length)(s[i], i);
                        if (x > width + scrollBox.scrollLeft) {
                            break;
                        }
                    }
                ...
                if (attributes.debug) {
                    ...
                    p = ('scrollLeft: %s, scrollTop: %s, topIndex: %s, topPixel: %s, leftIndex: %s, leftPixel: %s,'
                        ...
                        .replace('%s', currentCell
                            ? (('(x: %s, y: %s, context: %s, dragContext: %s, style: %s, type: %s)')
        ...
        function stopDragReorder(e) {
            ...
            ignoreNextClick = true;
            if (reorderObject.index !== reorderTarget.index
                    && !dispatchEvent('reorder', [e, reorderObject, reorderTarget], intf)) {
                //remove order number from the previous location
                columnOrder.splice(columnOrder.indexOf(reorderObject.index), 1);
                //place it in the new location
                columnOrder.splice(columnOrder.indexOf(reorderTarget.index), 0, reorderObject.index);
                //setStorageData();
            }
            draw(true);
        ...
        function init() {
            ...
            Object.defineProperty(intf, 'schema', {
                ...
                set: function schemaSetter(value) {
                    ...
                    createNewRowData();
                    resize(true);
                    columnOrder = fillArray(0, schema.length);
            ...
            Object.defineProperty(intf, 'data', {
                ...
                set: function dataSetter(value) {
                    ...
                    if (tempSchema) {
                        columnOrder = fillArray(0, tempSchema.length);
                    ...
                    if (!resize()) { draw(true); }
                    rowOrder = fillArray(0, data.length);
