...
(window.define || function defineStub(a, b) {
    ...
    function grid(args) {
        ...
        function position(e, canvasPosition) {
            var x = 0, y = 0, s = e, h, w;
            while (e.offsetParent) {
                ...
                h = e.offsetHeight;
                w = e.offsetWidth;
                if (e.offsetParent.tagName && canvasPosition) {
                    break;
                }
                e = e.offsetParent;
            }
        ...
        function dispatchEvent(ev, args, context) {
            ...
            events[ev].forEach(function dispatchEachEvent(fn) {
                args[0].preventDefault = function preventDefault() {
        ...
        function draw(internal) {
                    ...
                    if (!dispatchEvent('beforerendercell', [ctx, d[header.name], d, header, cx, cy], intf)) {
                        ...
                        if (cellStyle === 'rowHeaderCell' && attributes.tree) {
                            if (!dispatchEvent('rendertreearrow', [ctx, cell], intf)) {
                                treeArrowSize = drawTreeArrow(cell, 0 + style[cellStyle + 'PaddingLeft'], cy, 0);
                            }
                        }
            ...
            function drawRow(r) {
                ...
                treeHeight = (rowOpen ? sizes.rows['tree' + rd[uniqueId]] : 0);
            ...
            if (attributes.showPerformance || debugMessage) {
                ctx.fillStyle = 'black';
                ctx.strokeStyle = 'white';
                if (true) {
                    ctx.font = '13px sans-serif';
                    ctx.fillStyle = 'white';
                    fillRect(12, 19, width - 30, 13);
                    ctx.fillStyle = 'black';
                    fillText(('x: %s, y: %s, w: %s, h: %s, bw: %s, bh: %s, sw: %s, sh: %s, sl: %s, st: %s, sr: %s, f: %s, c: %s')
                        .replace('%s', mouse.x)
                        .replace('%s', mouse.y)
                        .replace('%s', width)
                        .replace('%s', height)
                        .replace('%s', scrollBox.width)
                        .replace('%s', scrollBox.height)
                        .replace('%s', scrollBox.scrollWidth)
                        .replace('%s', scrollBox.scrollHeight)
                        .replace('%s', scrollBox.scrollLeft)
                        .replace('%s', scrollBox.scrollTop)
                        .replace('%s', (scrollBox.height / scrollBox.scrollHeight) + '/'
                            + (scrollBox.width / scrollBox.scrollWidth))
                        .replace('%s', hasFocus)
                        .replace('%s', currentCell
                            ? (currentCell.rowIndex) : ''), 30, 30);
                } else {
                    p = (performance.now() - p).toFixed(2) + 'ms (' + visibleCells.length + ')';
                    ctx.font = '33px sans-serif';
                    fillText(p, w - (w / 5), h - (h / 10));
                    strokeText(p, w - (w / 5), h - (h / 10));
                }
            }
            ctx.restore();
        }
        function blurChildGrids() {
            Object.keys(childGrids).forEach(function (gridKey) {
                childGrids[gridKey].blur();
            });
        }
        ...
        function resize(drawAfterResize) {
            ...
            if (isChildGrid) {
                ...
                cPos = position(parentNode, true);
                canvasOffsetTop = cPos.top;
                canvasOffsetLeft = cPos.left;
                parentNode.offsetTop = cPos.top;
                parentNode.offsetLeft = cPos.left;
            } else {
            ...
            scrollHeight = data.reduce(function reduceData(accumulator, row) {
                return accumulator
                    ...
                    + (sizes.rows['tree' + row[uniqueId]] || 0)
                    + cellBorder;
            }, 0) || 0;
        ...
        function expandTree(rowIndex) {
            var headerCellHeight = getHeaderCellHeight(),
                ...
                h = sizes.rows['tree' + rowId] || style.treeGridHeight,
                treeGrid;
            if (!childGrids[rowId]) {
                treeGrid = grid({
            ...
            sizes.rows['tree' + data[rowIndex][uniqueId]] = style.treeGridHeight;
            dispatchEvent('resizerow', [style.cellHeight], intf);
            resize(true);
            draw();
        }
        ...
        function getCellAt(x, y) {
            ...
            for (i = 0; i < l; i += 1) {
                ...
                if (cell.x < x
                        ...
                        && cell.y < y
                        && cell.y + cell.height + attributes.borderResizeZone > y) {
        ...
        function dragResizeColumn(e) {
            ...
            if (resizeMode === 'ns-resize') {
                if (resizingItem.rowOpen) {
                    sizes.rows['tree' + resizingItem.data[uniqueId]] = y;
        ...
        function mousedown(e) {
            ...
            if (['ns-resize', 'ew-resize'].indexOf(resizeMode) !== -1) {
                ...
                if (resizingItem.rowOpen) {
                    resizingStartingHeight = sizes.rows['tree' + resizingItem.data[uniqueId]];
                } else {
