...
define([
    ...
    './contextMenu',
    './defaults'
], function context(xdraw, xevents, xintf, xcontextMenu, defaults) {
    'use strict';
    function grid(args) {
        args = args || {};
        var orders = {
        ...
        function getLayerPos(e) {
            ...
            if (isChildGrid) {
                pos.x -= canvasOffsetLeft;
                pos.y -= canvasOffsetTop;
            }
            return {
                x: pos.x,
                y: pos.y
        ...
        function createColumnOrders() {
            var s = getSchema();
            orders.columns = fillArray(0, s.length - 1);
        }
        function createRowOrders() {
            orders.rows = fillArray(0, data.length - 1);
        }
        ...
        function addEllipsis(text, width) {
            var o, i, c = style.cellPaddingRight + style.cellPaddingLeft;
        ...
        function addBorderLine(c, pos) {
            ...
            var p = {
                t: function () {
                    ctx.moveTo(c.x + canvasOffsetLeft, c.y + canvasOffsetTop);
                    ctx.lineTo(c.x + canvasOffsetLeft + c.width, c.y + canvasOffsetTop);
                },
                r: function () {
                    ctx.moveTo(c.x + canvasOffsetLeft + c.width, c.y + canvasOffsetTop);
                    ctx.lineTo(c.x + canvasOffsetLeft + c.width, c.y + canvasOffsetTop + c.height);
                },
                b: function () {
                    ctx.moveTo(c.x + canvasOffsetLeft, c.y + canvasOffsetTop + c.height);
                    ctx.lineTo(c.x + canvasOffsetLeft + c.width, c.y + canvasOffsetTop + c.height);
                },
                l: function () {
                    ctx.moveTo(c.x + canvasOffsetLeft, c.y + canvasOffsetTop);
                    ctx.lineTo(c.x + canvasOffsetLeft, c.y + canvasOffsetTop + c.height);
                },
            };
            p[pos]();
            ctx.stroke();
        ...
        function getHeaderWidth() {
            return getVisibleSchema().reduce(function (total, header) {
        ...
        function draw(internal) {
            ...
            if (data.length > orders.rows.length) {
                createRowOrders();
            }
            ...
            function drawCell(d, rowIndex, rowOrderIndex) {
                return function drawEach(header, headerIndex, columnOrderIndex) {
                    var cellStyle = header.style || 'cell',
                        childGridAttributes,
                        cell,
                        selected = selections[rowOrderIndex] && selections[rowOrderIndex].indexOf(columnOrderIndex) !== -1,
                        hovered = hovers[d[uniqueId]] && hovers[d[uniqueId]].indexOf(columnOrderIndex) !== -1,
                        active = activeCell.rowIndex === rowOrderIndex && activeCell.columnIndex === columnOrderIndex,
                        isGrid = Array.isArray(d[header.name]),
                        activeHeader = (orders.rows[activeCell.rowIndex] === rowOrderIndex
                                || orders.columns[activeCell.columnIndex] === columnOrderIndex)
                    ...
                    if (!dispatchEvent('beforerendercell', [{}, ctx, d[header.name], d, header, cx, cy], intf)) {
                        ...
                        if (activeHeader) {
                            ctx.fillStyle = style[cellStyle + 'ActiveBackgroundColor'];
                        }
                        ...
                        if ((attributes.showRowNumbers && cellStyle === 'rowHeaderCell')
                            ...
                            if (cell.isGrid) {
                                ...
                                if (childGrids[cell.gridId]) {
                                    childGrids[cell.gridId].parentNode.offsetHeight = 0;
                                }
                                ...
                                if (activeHeader) {
                                    ctx.fillStyle = style[cellStyle + 'ActiveColor'];
                                }
                                if (columnFilters && val in columnFilters && cellStyle === 'headerCell') {
                        ...
                        if (active && !attributes.rowSelectionMode) {
                            aCell = cell;
                        ...
                        if (selected) {
                            ctx.lineWidth = style.selectionOverlayBorderWidth;
                            ctx.strokeStyle = style.selectionOverlayBorderColor;
                            if (cell.selected && cell.style !== 'rowHeaderCell') {
                                if ((!selections[cell.rowIndex - 1]
                                        || selections[cell.rowIndex - 1].indexOf(cell.columnIndex) === -1
                                        || cell.rowIndex === 0)
                                        && !cell.isHeader) {
                                    addBorderLine(cell, 't');
                                }
                                if (!selections[cell.rowIndex] || cell.columnIndex === s.length - 1
                                        || selections[cell.rowIndex].indexOf(cell.columnIndex + 1) === -1) {
                                ...
                                if (!selections[cell.rowIndex] || cell.columnIndex === 0
                                        || selections[cell.rowIndex].indexOf(cell.columnIndex - 1) === -1) {
            ...
            function drawHeaderRow() {
                ...
                if (attributes.showHeaders) {
                    x = (scrollBox.scrollLeft * -1) + scrollPixelLeft;
                    ...
                    for (o = scrollIndexLeft; o < g; o += 1) {
                        ...
                        if (x > width + scrollBox.scrollLeft) {
                            break;
                        }
                    }
            ...
            function drawRows() {
                ...
                if (attributes.showNewRow) {
                    ...
                    for (o = scrollIndexLeft; o < g; o += 1) {
                        i = orders.columns[o];
                        x += drawCell(newRow, data.length, data.length)(s[i], i, o);
                        if (x > width + scrollBox.scrollLeft) {
                            break;
                        }
                    }
                    drawRowHeader(newRow, data.length, data.length);
                }
            }
            function drawReorderMarkers() {
                if (!reorderObject) { return; }
                var b = {
                        height: reorderObject.height,
                        width: reorderObject.width,
                        x: reorderObject.x + reorderObject.dragOffset.x,
                        y: reorderObject.y + reorderObject.dragOffset.y
                    },
                    m = {
                        width: w,
                        heigth: h,
                ...
                ctx.fillStyle = style.reorderMarkerBackgroundColor;
                ctx.lineWidth = style.reorderMarkerBorderWidth;
                ctx.strokeStyle = style.reorderMarkerBorderColor;
                if (dragMode === 'row-reorder') {
                    b.width = w;
                    b.x = 0;
                    m.width = w;
                    m.y = currentCell.y;
                    ...
                    ctx.lineWidth = style.reorderMarkerIndexBorderWidth;
                    ctx.strokeStyle = style.reorderMarkerIndexBorderColor;
                    if (currentCell.rowIndex !== reorderObject.rowIndex
                            && currentCell.rowIndex - 1 !== reorderObject.rowIndex) {
                        addBorderLine(m, 't');
                    }
                } else if (dragMode === 'column-reorder' && reorderObject) {
                    b.height = h;
                    b.y = 0;
                    m.height = h;
                    m.x = currentCell.x;
                    ...
                    ctx.lineWidth = style.reorderMarkerIndexBorderWidth;
                    ctx.strokeStyle = style.reorderMarkerIndexBorderColor;
                    if (currentCell.columnIndex !== reorderObject.columnIndex
                            && currentCell.columnIndex - 1 !== reorderObject.columnIndex) {
                        addBorderLine(m, 'l');
                    }
                }
            }
            function drawBorder() {
                ctx.lineWidth = style.gridBorderWidth;
                ctx.strokeStyle = style.gridBorderColor;
            ...
            function drawDebug() {
                ...
                if (attributes.debug) {
                    ...
                    p = ('scrollLeft: %s, scrollTop: %s, topIndex: %s, topPixel: %s, leftIndex: %s, leftPixel: %s,'
                        ...
                        + '\ncontext: %s')
                        .replace('%s', scrollBox.scrollLeft)
                        .replace('%s', scrollBox.scrollTop)
                        .replace('%s', scrollIndexTop)
                        .replace('%s', scrollPixelTop)
                        .replace('%s', scrollIndexLeft)
                        .replace('%s', scrollPixelLeft)
                        ...
                        .replace('%s', currentCell
                            ? (('(x: %s, y: %s, ox: %s, oy: %s, context: %s, dragContext: %s, style: %s, type: %s)')
                                ...
                                .replace('%s', currentCell.sortColumnIndex)
                                .replace('%s', currentCell.sortRowIndex)
                                .replace('%s', currentCell.context)
                                .replace('%s', currentCell.dragContext)
                                .replace('%s', currentCell.style)
                                .replace('%s', currentCell.type)) : '');
                    p.split('\n').forEach(function (s, index) {
                        ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
                        fillRect(70, 39 + (index * 15), ctx.measureText(s).width + 20, 15);
        ...
        function resize(drawAfterResize) {
            ...
            if (isChildGrid) {
                ...
                height = parentDOMNode.offsetHeight - style.scrollBarBoxMargin;
                width = parentDOMNode.offsetWidth - style.scrollBarBoxMargin;
            ...
            scrollBox.widthBoxRatio = ((scrollBox.width - scrollDragPositionOffsetX)
                / (scrollBox.scrollWidth + scrollBox.width - scrollDragPositionOffsetX));
            scrollBox.scrollBoxWidth = scrollBox.width
                * scrollBox.widthBoxRatio
                - style.scrollBarWidth;
            scrollBox.heightBoxRatio = ((scrollBox.height - scrollDragPositionOffsetY)
                / (scrollBox.scrollHeight + (scrollBox.height - scrollDragPositionOffsetY)));
        ...
        function scroll(e) {
            ...
            while (scrollPixelLeft < scrollBox.scrollLeft && scrollIndexLeft < s.length) {
                scrollPixelLeft +=
                    (sizes.columns[s[scrollIndexLeft][uniqueId]] || s[scrollIndexLeft].width);
                scrollIndexLeft += 1;
            }
            if (data.length > 0) {
                scrollIndexLeft = Math.max(scrollIndexLeft - 1, 0);
                scrollPixelLeft = Math.max(scrollPixelLeft
                    - (sizes.columns[s[scrollIndexLeft][uniqueId]] || s[scrollIndexLeft].width), 0);
        ...
        function getCellAt(x, y) {
            ...
            for (i = 0; i < l; i += 1) {
                cell = visibleCells[i];
                if (cell.x - style.cellBorderWidth < x
                        && cell.x + cell.width + style.cellBorderWidth > x
                        && cell.y - style.cellBorderWidth < y
        ...
        function mousemove(e) {
            ...
            o = getCellAt(x, y);
            if (o && currentCell && (currentCell.rowIndex !== o.rowIndex
                    || currentCell.columnIndex !== o.columnIndex)) {
                cellBoundaryCrossed = true;
                dispatchEvent('cellmouseover', [e, o], intf);
                dispatchEvent('cellmouseout', [e, currentCell], intf);
            }
            currentCell = o;
            if (!hasFocus) {
                return;
            ...
            if (!draggingItem
                ...
                dragItem = o;
                dragMode = o.dragContext;
                canvas.style.cursor = o.context;
                if (o.context === 'cell' && o.data) {
                    canvas.style.cursor = 'pointer';
                    hovers[o.data[uniqueId]] = [o.rowIndex];
                }
                if ((selecting || reorderObject)
                        && o.context === 'cell'
                        && o.data) {
                    sBounds = getSelectionBounds();
                    delta = {
                        x: Math.abs(dragStart.x - x),
                        y: Math.abs(dragStart.y - y),
                    };
                    if (dragStartObject.columnIndex !== -1 && e.shiftKey) {
                        dragStartObject = {
                    ...
                    if (dragStartObject.columnIndex === -1) {
                        dragBounds.left = -1;
                    ...
                    if (cellBoundaryCrossed || (delta.x === 0 && delta.y === 0) || attributes.rowSelectionMode) {
                        if (attributes.rowSelectionMode || dragStartObject.columnIndex === -1) {
                            ...
                            if (!dragAddToSelection && o.rowIndex) {
                                ...
                                if (selections[o.rowIndex].indexOf(o.columnIndex) === -1) {
        ...
        function autoScrollZone(e, x, y, ctrl) {
            ...
            if (setTimer && !ctrl && currentCell && currentCell.columnIndex !== -1) {
        ...
        function setFilter(column, value) {
            ...
            Object.keys(columnFilters).forEach(function (filter) {
                ...
                data = data.filter(function (row) {
                    return currentFilter(row[filter], columnFilters[filter]);
                });
            });
            resize();
            draw(true);
        ...
        function contextmenu(e) {
            ...
            if (attributes.showFilter) {
                ...
                if (Object.keys(columnFilters).length) {
                    Object.keys(columnFilters).forEach(function (cf) {
                        menuItems.push({
                            title: attributes.removeFilterOptionText.replace(/%s/g, cf),
            ...
            if (attributes.saveAppearance && attributes.showClearSettingsOption
                    && (Object.keys(sizes.rows).length > 0
                        || Object.keys(sizes.columns).length > 0)) {
                menuItems.push({
                    title: attributes.clearSettingsOptionText,
                    click: function (e) {
                        e.preventDefault();
                        sizes.rows = {};
                        sizes.columns = {};
            ...
            if (attributes.allowSorting && attributes.showOrderByOption) {
                menuItems.push({
                    title: attributes.showOrderByOptionTextAsc.replace('%s', contextObject.header.name),
                    click: function (e) {
                        e.preventDefault();
                ...
                menuItems.push({
                    title: attributes.showOrderByOptionTextDesc.replace('%s', contextObject.header.name),
                    click: function (e) {
                        e.preventDefault();
        ...
        function beginEditAt(x, y) {
            ...
            setActiveCell(x, y);
            function postDraw() {
                var pos = position(parentNode, true);
                cell = visibleCells.filter(function (vCell) {
                    return vCell.columnIndex === x && vCell.rowIndex === y;
                })[0];
                ...
                input.style.top = pos.top - 1 + top + 'px';
                input.style.left = pos.left - 1 + left + 'px';
        ...
        function click(e) {
            ...
            if (currentCell.context === 'cell') {
                ...
                if (['rowHeaderCell', 'headerCell'].indexOf(currentCell.style) === -1 && !ctrl) {
        ...
        function dragResizeColumn(e) {
            ...
            if (dispatchEvent('resizecolumn', [{}, x, y, draggingItem], intf)) { return false; }
            if (scrollBox.scrollLeft > scrollBox.scrollWidth - attributes.resizeScrollZone
                    && dragMode === 'ew-resize') {
                resize(true);
                scrollBox.scrollLeft += x;
            }
            if (dragMode === 'ew-resize') {
                sizes.columns[draggingItem.header.style === 'rowHeaderCell'
                       ? 'cornerCell' : draggingItem.header[uniqueId]] = x;
            ...
            if (dragMode === 'ns-resize') {
                if (draggingItem.rowOpen) {
                    sizes.trees[draggingItem.data[uniqueId]] = y;
                } else if (attributes.globalRowResize) {
                    style.cellHeight = y;
                } else {
                    sizes.rows[draggingItem.data[uniqueId]] = y;
                }
        ...
        function dragReorder(e) {
            ...
            x = pos.x - dragStart.x;
            y = pos.y - dragStart.y;
            if (!attributes.allowColumnReordering && dragMode === 'column-reorder') {
                return;
            }
            if (!attributes.allowRowReordering && dragMode === 'row-reorder') {
                return;
            }
            if (dispatchEvent('reordering', [e, dragStartObject, currentCell, dragMode], intf)) {
                return;
            }
            if (Math.abs(x) > attributes.reorderDeadZone || Math.abs(y) > attributes.reorderDeadZone) {
                reorderObject = dragStartObject;
                reorderTarget = currentCell;
                reorderObject.dragOffset = {
                    x: x,
                    y: y
                };
        ...
        function stopDragReorder(e) {
            var cr = {
                    'row-reorder': orders.rows,
                    'column-reorder': orders.columns
                },
                i = {
            ...
            if (reorderObject
                    && reorderTarget) {
                ignoreNextClick = true;
                if (reorderObject[i] !== reorderTarget[i]
                        && !dispatchEvent('reorder', [e, reorderObject, reorderTarget, dragMode], intf)) {
                    cr[dragMode].splice(cr[dragMode].indexOf(reorderObject[i]), 1);
                    cr[dragMode].splice(cr[dragMode].indexOf(reorderTarget[i]), 0, reorderObject[i]);
                    setStorageData();
                }
            }

            reorderObject = undefined;
        ...
        function mousedown(e) {
            ...
            dragAddToSelection = !dragStartObject.selected;
            if (!ctrl && !e.shiftKey && !/(vertical|horizontal)-scroll-(bar|box)/
                    .test(dragStartObject.context)) {
                selections = [];
            }
            ...
            if (['ns-resize', 'ew-resize'].indexOf(dragMode) !== -1) {
                draggingItem = dragItem;
                if (draggingItem.rowOpen) {
                    resizingStartingHeight = sizes.trees[draggingItem.data[uniqueId]];
                } else {
                    resizingStartingHeight = sizes.rows[draggingItem.data[uniqueId]] || style.cellHeight;
                }
                resizingStartingWidth = sizes.columns[draggingItem.header.style === 'rowHeaderCell'
                       ? 'cornerCell' : draggingItem.header[uniqueId]] || draggingItem.header.width;
            ...
            if (['row-reorder', 'column-reorder'].indexOf(dragMode) !== -1) {
                draggingItem = dragItem;
        ...
        function keydown(e) {
            var i,
                x = activeCell.columnIndex,
                y = activeCell.rowIndex,
            ...
            if (e.keyCode === 32) {
                selections = [];
                selections[Math.max(y, 0)] = [];
                selections[Math.max(y, 0)].push(x);
                selectionBounds = getSelectionBounds();
                if (attributes.rowSelectionMode) {
            ...
            if (e.shiftKey && [37, 38, 39, 40].indexOf(e.keyCode) !== -1) {
                selections[Math.max(y, 0)] = selections[Math.max(y, 0)] || [];
                selections[Math.max(y, 0)].push(x);
                selectionBounds = getSelectionBounds();
                selectArea(undefined, ctrl);
                draw(true);
            }
            if (x !== activeCell.columnIndex || y !== activeCell.rowIndex) {
                scrollIntoView(x !== activeCell.columnIndex ? x : undefined, y !== activeCell.rowIndex ? y : undefined);
                setActiveCell(x, y);
        ...
        function setAttributes() {
            defaults.attributes.forEach(function eachAttribute(i) {
                setDefaults(attributes, args, i[0], i[1]);
            });
        }
        function setStyle() {
            defaults.styles.forEach(function eachStyle(i) {
                setDefaults(style, args.style || {}, i[0], i[1]);
            });
        }
            ...
            var styleSheet,
                ...
                css = {
                    ...
                    'canvas-datagrid-edit-input': {
                        ...
                        'box-shadow': style.editCellBoxShadow,
                        border: style.editCellBorder
                    },
        ...
        function appendTo(n) {
            ...
            if (MutationObserver) {
                ...
                [canvas.parentNode].forEach(function (el) {
        ...
        function init() {
            ...
            Object.defineProperty(intf, 'dragMode', {
                get: function () {
                    return dragMode;
                }
            });
            ...
            Object.defineProperty(intf, 'data', {
                ...
                set: function dataSetter(value) {
                    ...
                    if (!schema && data.length === 0) {
                        tempSchema = [{name: ''}];
                        tempSchema[0][uniqueId] = getSchemaNameHash('');
                    }
            ...
            if (attributes.name && attributes.saveAppearance) {
                ...
                if (storedSettings) {
                    if (typeof storedSettings.sizes === 'object') {
