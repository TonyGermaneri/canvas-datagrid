    ...
    function grid(args) {
                ...
                if (selections.length > 0 || aCell) {
                    ...
                    if (selections.length > 0) {
                        ...
                        visibleCells.forEach(function (c) {
                            if (c.selected && c.style !== 'rowHeaderCell') {
                                if (!selections[c.rowIndex - 1]
                                        || selections[c.rowIndex - 1].indexOf(c.columnIndex) === -1) {
                                    addBorderLine(c, 't');
                                }
                                if (!selections[c.rowIndex] || c.columnIndex === s.length - 1
                                        || selections[c.rowIndex].indexOf(c.columnIndex + 1) === -1) {
                                    addBorderLine(c, 'r');
                                }
                                if (!selections[c.rowIndex + 1]
                                        || selections[c.rowIndex + 1].indexOf(c.columnIndex) === -1) {
                                ...
                                if (!selections[c.rowIndex] || c.columnIndex === 0
                                        || selections[c.rowIndex].indexOf(c.columnIndex - 1) === -1) {
                                    addBorderLine(c, 'l');
                                }
                            }
                    ...
                    if (!dispatchEvent('beforerendercell', [ctx, d[header.name], d, header, cx, cy], intf)) {
                        ...
                        if ((attributes.showRowNumbers && cellStyle === 'rowHeaderCell')
                            ...
                            if (cell.isGrid) {
                                ...
                                cell.formattedValue = (val !== undefined ? val : '').toString();
            ...
            if (!resizingItem
                ...
                if (selecting
                    ...
                    if (dragStartObject.columnIndex === -1) {
                        dragBounds.left = -1;
                        dragBounds.right = schema.length - 1;
                        dragBounds.top = Math.min(sBounds.top, o.rowIndex);
                        dragBounds.bottom = Math.max(sBounds.bottom, o.columnIndex);
                    }
        ...
        function click(e) {
            ...
            if (currentCell.context === 'cell') {
                ...
                if ((attributes.rowSelectionMode || currentCell.style === 'rowHeaderCell') && !ctrl) {
