    ...
    function grid(args) {
        ...
        function draw(internal) {
            ...
            var checkScrollHeight, borderWidth, rowHeaderCell, p, cx, cy, treeGrid, rowOpen,
                rowHeight, cornerCell, y, x, c, h, w, s, r, rd,
                l = data.length,
                u = currentCell || {},
            ...
            y = parentNode.offsetTop + (scrollBox.scrollTop * -1) + headerCellHeight + scrollPixelTop;
            h = parentNode.offsetHeight;
            w = parentNode.offsetWidth;
            ctx.save();
            radiusRect(
            ...
            function drawScrollBars() {
                ...
                vb.y = parentNode.offsetTop + headerCellHeight + style.scrollBarBoxMargin
                ...
                nb.x = parentNode.offsetLeft + headerCellWidth + style.scrollBarBoxMargin
                ...
                return function drawEach(header, headerIndex) {
                    ...
                    if (!dispatchEvent('beforerendercell', [ctx, d[header.name], d, header, cx, cy], intf)) {
                        ...
                        if (cellStyle === 'cornerCell') {
                            cx = parentNode.offsetLeft;
                            cy = parentNode.offsetTop;
                        ...
                        if (cellStyle === 'rowHeaderCell' && attributes.tree) {
                            if (!dispatchEvent('rendertreearrow', [ctx, cell], intf)) {
                                treeArrowSize = drawTreeArrow(cell, parentNode.offsetLeft + style[cellStyle + 'PaddingLeft'], cy, 0);
                            }
                        }
                            ...
                            if (cell.isGrid) {
                                ...
                                if (cellStyle === 'headerCell' && orderBy === header.name) {
                                    if (!dispatchEvent('renderorderbyarrow', [ctx, cell], intf)) {
                                        orderByArrowSize = drawOrderByArrow(cx + style[cellStyle + 'PaddingLeft'], parentNode.offsetTop);
                                    }
                                }
                        ...
                        return (x - parentNode.offsetLeft - cell.width > parentNode.offsetWidth + scrollBox.scrollLeft);
                    }
                };
            }
            ...
            function drawRow(r) {
                var u, g = s.length;
                if (y - parentNode.offsetTop - (cellHeight * 2) > h) {
                    return false;
                }
                rd = data[r];
                ...
                if (y - parentNode.offsetTop < cellHeight * -1) {
                    return false;
                }
                if (attributes.showRowHeaders) {
                    x = parentNode.offsetLeft + headerCellWidth;
                }
                for (u = 0; u < g; u += 1) {
                ...
                if (r !== data.length && rowOpen) {
                    ...
                    treeGrid.parentNode = {
                        offsetTop: y + treeGrid.originalRowHeight,
                        offsetLeft: x + headerCellWidth - 1,
                        offsetHeight: cellHeight - treeGrid.originalRowHeight,
            ...
            if (attributes.showPerformance) {
                ...
                p = (performance.now() - p).toFixed(2) + 'ms (' + visibleCells.length + ')';
        ...
        function resize() {
            if (dispatchEvent('resize', [], intf)) { return false; }
            if (!isChildGrid) {
                canvas.height = height;
                canvas.width = width;
                parentNode.offsetHeight = parentDOMNode.offsetHeight;
                parentNode.offsetWidth = parentDOMNode.offsetWidth;
            }
        ...
        function getClippingRect(ele) {
            var boundingRect = position(canvas),
        ...
        function expandTree(rowIndex) {
            ...
            if (!childGrids[rowId]) {
                treeGrid = grid({
                    ...
                    parentNode: {
                        ...
                        offsetHeight: h,
                        offsetWidth: parentNode.offsetWidth - headerCellWidth,
                        header: { width: parentNode.offsetWidth - headerCellWidth },
            ...
            treeGrid.visible = true;
            treeGrid.originalRowHeight = sizes.rows[data[rowIndex][uniqueId]] || style.cellHeight;
            dispatchEvent('expandtree', [treeGrid, data[rowIndex], rowIndex], intf);
            openChildren[data[rowIndex][uniqueId]] = treeGrid;
            sizes.rows[data[rowIndex][uniqueId]] = style.treeGridHeight + treeGrid.originalRowHeight;
            dispatchEvent('resizerow', [style.cellHeight], intf);
            resize();
            draw();
        }
        ...
        function scrollGrid(e) {
            ...
            if (scrollMode === 'vertical-scroll-box') {
                scrollBox.scrollTop = (scrollBox.scrollHeight *
                    ((pos.y - parentNode.offsetTop + scrollBox.scrollBoxHeight - vOffset)
                        / (height - vOffset)));
            ...
            if (scrollMode === 'horizontal-scroll-box') {
                scrollBox.scrollLeft = (scrollBox.scrollWidth *
                    ((pos.x - parentNode.offsetLeft + scrollBox.scrollBoxHeight - hOffset)
                        / (width - hOffset)));
        ...
        function isInGrid(e) {
            if (e.x < 0
                    || e.x > parentNode.offsetWidth
                    || e.y < 0
                    || e.y > parentNode.offsetHeight) {
                return false;
            }
            return true;
        }
        ...
        function appendTo(n) {
            ...
            if (parentNode && /canvas-datagrid-(cell|tree)/.test(parentNode.nodeType)) {
                ...
                parentGrid.controlInput.parentNode.appendChild(controlInput);
            } else {
            ...
            canvas.addEventListener('mousewheel', scrollWheel, false);
        ...
        function setDom() {
            ...
            controlInput.addEventListener('blur', function () {
                hasFocus = false;
            }, false);
