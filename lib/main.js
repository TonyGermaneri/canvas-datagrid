...
(window.define || function defineStub(a, b) {
    ...
    function grid(args) {
        ...
        function draw() {
                    ...
                    if (!dispatchEvent('beforerendercell', [ctx, d[header.name], d, header, cx, cy], intf)) {
                        ...
                        if ((attributes.showRowNumbers && cellStyle === 'rowHeaderCell')
                            ...
                            if (cell.isGrid) {
                                ...
                                cell.grid.childGridContainer.style.top = canvas.getBoundingClientRect().top + cell.y + 'px';
                                cell.grid.childGridContainer.style.left = canvas.getBoundingClientRect().left + cell.x + 'px';
            ...
            function drawRow(r) {
                ...
                if (r !== data.length - 1 && rowOpen) {
                    if (!childGrids[rd[uniqueId]]) {
                        treeGridAttributes = openChildren[rd[uniqueId]];
                        treeGridAttributes.name =
                            treeGridAttributes.name || attributes.saveAppearance
                                ? 'tree' + rd[uniqueId] : undefined;
                        treeGridAttributes.height = sizes.rows[rd[uniqueId]] || style.treeGridHeight;
                        treeGridAttributes.width = canvas.offsetWidth - headerCellWidth;
                        treeGridAttributes.parentNode = {
                            parentGrid: intf,
                            container: container,
                            height: sizes.rows[rd[uniqueId]] || style.treeGridHeight - style.cellHeight
                                - headerCellHeight,
                        ...
                        childGrids[rd[uniqueId]] = grid(treeGridAttributes);
                    }
                    treeGrid = childGrids[rd[uniqueId]];
                    treeGrid.visible = true;
                    treeGrid.childGridContainer.style.top =
                        canvas.getBoundingClientRect().top + y + style.cellHeight + 'px';
                    treeGrid.childGridContainer.style.left =
                        canvas.getBoundingClientRect().left + x + headerCellWidth + 'px';
            ...
            if (attributes.showHeaders) {
                ...
                // cell height might have changed during drawing
                cellHeight = sizes.rows[-1] || style.headerCellHeight;
                s.forEach(function forEachHeader(header, index) {
                    var d = {
                        title: header.title,
                        name: header.name,
        ...
        function resize() {
            var pos = isChildGrid ? {left: 0, top: 0} : canvas.getBoundingClientRect(),
                h,
                headerCellHeight = sizes.rows[-1] || style.headerCellHeight,
                headerCellWidth = sizes.columns.cornerCell || style.headerRowWidth,
                headerCellBorder =  style.headerCellBorderWidth * 2;
            ...
            scrollBox.style.top = pos.top + headerCellHeight + headerCellBorder
                - style.scrollDivOverlap + 'px';
            scrollBox.style.left = pos.left + headerCellWidth + 'px';
            draw();
            return true;
        }
        function clipElement(ele) {
            //TODO BUG: Clip on child grids just doesn't work.  Bad position references.  HTML is hard.
            var boundingRect = isChildGrid ? parentGrid.canvas : canvas.getBoundingClientRect(),
                clipRect = {
                    x: 0,
                    y: 0,
                    h: 0,
                ...
                headerCellHeight = sizes.rows[-1] || style.headerCellHeight,
                headerCellWidth = sizes.columns.cornerCell || style.headerRowWidth;
            clipRect.h = ele.offsetHeight;
            clipRect.w = ele.offsetWidth;
            clipRect.x = boundingRect.left + (ele.offsetLeft * -1) + headerCellWidth;
            clipRect.y = boundingRect.top + (ele.offsetTop * -1) + headerCellHeight;
        ...
        function collapseTree(rowIndex) {
            ...
            dispatchEvent('collapsetree', [e], intf);
            openChildren[data[rowIndex][uniqueId]] = undefined;
            sizes.rows[data[rowIndex][uniqueId]] = undefined;
        ...
        function expandTree(rowIndex) {
            ...
            dispatchEvent('expandtree', [e], intf);
            openChildren[data[rowIndex][uniqueId]] = e.grid;
            sizes.rows[data[rowIndex][uniqueId]] = style.treeGridHeight;
            dispatchEvent('resizerow', [style.cellHeight], intf);
            setScrollArea();
            draw();
        }
        ...
        function scroll() {
            ...
            Object.keys(childGrids).forEach(function (gridId) {
                clipElement(childGrids[gridId].childGridContainer);
            });
            dispatchEvent('scroll', [getSelectedData(), selections, selectionBounds], intf);
        }
        ...
        function dragResizeColumn(e) {
            ...
            if (resizeMode === 'ew-resize') {
                ...
                dispatchEvent('resizecolumn', [x], intf);
                return;
            }
        ...
        function attachCss() {
            var styleSheet, css = [
                ...
                '.canvas-datagrid { box-sizing: content-box!important; padding: 0!important; }',
            ...
            function resizeChildGrid() {
                if (cell.style === 'tree') {
                    childGridContainer.style.width = parentGrid.canvas.offsetWidth
                        - headerCellWidth - parentGrid.style.scrollBarWidth + 'px';
                    childGridContainer.style.height = (parentGrid.sizes.rows[cell.data[uniqueId]]
                        || parentGrid.style.treeGridHeight) - parentGrid.style.cellBorderWidth -
                        parentGrid.style.cellHeight + 'px';
                } else {
                    ...
                    childGridContainer.style.height = (parentGrid.sizes.rows[cell.data[uniqueId]]
                        || parentGrid.style.cellHeight) - parentGrid.style.cellBorderWidth + 'px';
                }
                args.parentNode = childGridContainer;
                requestAnimationFrame(resize);
            }
            cell.container.appendChild(childGridContainer);
        ...
        function setDom() {
            ...
            controlInput.style.display = 'fixed';
            controlInput.style.top = '-30px';
