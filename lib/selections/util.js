 ...
 * @param {boolean} [alwaysArray=false] Don't return boolean value but always an array.
...
const areCellsSelected = (
  ...
  if (!selections || selections.length === 0) return alwaysArray ? [] : false;
  let breakLoop;
  for (let i = 0, iTo = alwaysArray ? 0 : selections.length; i < iTo; i++) {
    const sel = selections[i];
    switch (sel.type) {
      case SelectionType.UnselectedCells:
      case SelectionType.Cells:
