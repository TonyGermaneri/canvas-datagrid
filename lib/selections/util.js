...
const areSelectionsNeat = (selections) => {
  ...
  for (let i = 0; i < selections.length; i++) {
    ...
    if (baseType === SelectionType.Cells) {
      if (sel.col0 === base.col0 && sel.col1 === base.col1) continue;
      if (sel.row0 === base.row0 && sel.row1 === base.row1) continue;
...
const moveSelections = (selections, offsetX, offsetY) => {
  for (let i = 0; i < selections.length; i++) {
    ...
    switch (selection.type) {
      case SelectionType.Cells:
      case SelectionType.UnselectedCells:
        selection.row0 += offsetY;
        selection.row1 += offsetY;
        selection.col0 += offsetX;
        selection.col1 += offsetX;
        break;
      case SelectionType.Rows:
        selection.row0 += offsetY;
        selection.row1 += offsetY;
...
const getSelectionBounds = (selections) => {
  ...
  for (let i = 0; i < selections.length; i++) {
    ...
    if (typeof sel.col0 !== 'number') left = 0;
    else if (sel.col0 < left) left = sel.col0;

    if (typeof sel.col1 !== 'number') right = Infinity;
    else if (sel.col1 > right) right = sel.col1;

    if (typeof sel.row0 !== 'number') top = 0;
    else if (sel.row0 < top) top = sel.row0;

    if (typeof sel.row1 !== 'number') bottom = Infinity;
    else if (sel.row1 > bottom) bottom = sel.row1;
  }
...
const shrinkOrExpandSelections = (selections, cell, keyEvent, context) => {
  ...
  for (let i = 0; i < selections.length; i++) {
    ...
    switch (sel.type) {
      ...
      case SelectionType.UnselectedCells:
        if (
          (originalRowIndex === sel.row0 || originalRowIndex === sel.row1) &&
          (originalColumnIndex === sel.col0 || originalColumnIndex === sel.col1)
        ) {
        ...
        break;
      case SelectionType.Rows:
        if (originalRowIndex === sel.row0 || originalRowIndex === sel.row1)
          matchedSelections.push(sel);
        break;
      case SelectionType.Columns:
        if (
          originalColumnIndex === sel.col0 ||
          originalColumnIndex === sel.col1
        )
          matchedSelections.push(sel);
        break;
    }
  ...
  const isTopBorder = selection.row0 === originalRowIndex;
  const isBottomBorder = selection.row1 === originalRowIndex;

  const isLeftBorder = selection.col0 === originalColumnIndex;
  const isRightBorder = selection.col1 === originalColumnIndex;
  ...
  if (isTopBorder) {
    if (keyInfo.isUp) {
      // at the topmost row
      if (selection.row0 === 0) return false;
      // expand to the up
      selection.row0--;
      return afterChange();
    } else if (keyInfo.isDown && selection.row0 < selection.row1) {
      selection.row0++;
      return afterChange();
    }
  }
  if (isBottomBorder) {
    if (keyInfo.isDown) {
      ...
      if (selection.row1 >= maxRow) return false;
      // expand to the down
      selection.row1++;
      return afterChange();
    } else if (keyInfo.isUp && selection.row0 < selection.row1) {
      selection.row1--;
      return afterChange();
    }
  }
  if (isLeftBorder) {
    if (keyInfo.isLeft) {
      ...
      if (selection.col0 === 0) return false;
      // expand to the left
      selection.col0--;
      return afterChange();
    } else if (keyInfo.isRight && selection.col0 < selection.col1) {
      selection.col0++;
      return afterChange();
    }
  }
  if (isRightBorder) {
    if (keyInfo.isRight) {
      ...
      if (selection.col1 >= maxColumn) return false;
      // expand to the down
      selection.col1++;
      return afterChange();
    } else if (keyInfo.isLeft && selection.col0 < selection.col1) {
      selection.col1--;
      return afterChange();
    }
  }
