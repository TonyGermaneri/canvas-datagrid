 ...
 * @param {boolean} [args.tree=false] - When true, an arrow will be drawn on each row that when clicked raises the [expandtree](https://tonygermaneri.github.io/canvas-datagrid/docs/#canvasDatagrid#event:expandtree) event for that row and creates an inner grid.  [See tree tutorials.](https://tonygermaneri.github.io/canvas-datagrid/docs/#tutorial--Allow-users-to-open-trees).
 ...
 * @param {object} [args.cellGridAttributes] - Attributes used for cell grids. These child grids are different than the tree grids. See the [data](https://tonygermaneri.github.io/canvas-datagrid/docs/#canvasDatagrid.data) property for more information about cell grids.
 * @param {boolean} [args.showNewRow=true] - When true, a row will appear at the bottom of the data set. schema[].defaultValue will define a default value for each cell. defaultValue can be a string or a function. When a function is used, the arguments header and index will be passed to the function. The value returned by the function will be the value in the new cell.  See [defaultValue](https://tonygermaneri.github.io/canvas-datagrid/docs/#canvasDatagrid.header.defaultValue)
 ...
 * @param {boolean} [args.multiLine=false] - When true, edit cells will be textareas, when false edit cells will be inputs.  See [use a textarea tutorial](https://tonygermaneri.github.io/canvas-datagrid/docs/#tutorial--Use-a-textarea-to-edit-cells-instead-of-an-input.)
 ...
 * @param {boolean} [args.schema=[]] - Sets the schema. See [schema](https://tonygermaneri.github.io/canvas-datagrid/docs/#schema).
 * @param {boolean} [args.data=[]] - Sets the data. See [data](https://tonygermaneri.github.io/canvas-datagrid/docs/#canvasDatagrid.data).
 ...
 * @param {string} [args.snapToRow=false] - When true, scrolling snaps to the top row.
 * @param {string} [args.touchZoomSensitivity=0.005] - The scale at which "pinch to zoom" screen pixels are converted to scale values.
 * @param {string} [args.touchZoomMin=0.5] - The minimum zoom scale.
 * @param {string} [args.touchZoomMax=1.75] - The maximum zoom scale.
 * @param {canvasDatagrid.style} [args.style={}] - Sets selected values in style.
 * @param {canvasDatagrid.filter} [args.filters={}] - Sets selected values in filters.  See [filters](https://tonygermaneri.github.io/canvas-datagrid/docs/#filters).
 * @param {canvasDatagrid.sorter} [args.sorters={}] - Sets selected values in sorters.  See [sorters](https://tonygermaneri.github.io/canvas-datagrid/docs/#sorters).
 * @param {canvasDatagrid.formatter} [args.formatters={}] - Sets selected values in formatters.  See [formatters](https://tonygermaneri.github.io/canvas-datagrid/docs/#formatters).
 ...
 * @property {object} attributes - Object that contains the properties listed in the attributes section.  These properties can be used at runtime to alter attributes set during instantiation.  See the [parameters and attributes](https://tonygermaneri.github.io/canvas-datagrid/docs/#params) section for full documentation.
 ...
 * @property {canvasDatagrid.filter} filters - Object that contains a list of filters for filtering data.  The properties in this object match the `schema[].type` property.  For example, if the schema for a given column was of the type `date` the grid would look for a filter called `filters.date` if a filter cannot be found for a given data type a warning will be logged and the string/RegExp filter will be used.   See [filters](https://tonygermaneri.github.io/canvas-datagrid/docs/#filters).
 * @property {canvasDatagrid.data} data - This is how data is set in the grid.  Data must be an array of objects that conform to a schema.  Data values can be any primitive type.  However if a cell value is another data array, an inner grid will be rendered into the cell.  This "cell grid" is different than a "tree grid" (the sort you drill in with a row header arrow) and uses the `cellGridAttributes` attribute for properties and styling. See [data](https://tonygermaneri.github.io/canvas-datagrid/docs/#canvasDatagrid.data).
 * @property {canvasDatagrid.schema} schema - Schema is optional.  Schema is an array of {canvasDatagrid.header} objects.  If no schema is provided one will be generated from the data, in that case all data will be assumed to be string data. See [schema](https://tonygermaneri.github.io/canvas-datagrid/docs/#schema).
 ...
 * @property {string} formattedValue - The value after passing through any formatters.  See [formatters](https://tonygermaneri.github.io/canvas-datagrid/docs/#formatters).
