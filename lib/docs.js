/**
 * Canvas Datagrid is a hierarchical canvas based web component.  The grid can be created in a number of ways.<br>
 * <ul><li>Web component by using the tag `&lt;canvas-datagrid&gt;&lt;/canvas-datagrid&gt;` anywhere in your document.</li>
 * <li>Web component by running `var foo = document.createElement('canvas-datagrid')`.<br>See more: {@link https://tonygermaneri.github.io/canvas-datagrid/docs/tutorial-webcomponent.html}.</li>
 * <li>Webpack3 universal module loader by adding one of many module loaders to your application code.  <br>See example: {@link https://tonygermaneri.github.io/canvas-datagrid/tutorials/amdDemo.html}.</li>
 * <li>You can also load the grid by invoking the global method `var foo = canvasDatagrid(&lt;args&gt;);` <br>See example: {@link https://tonygermaneri.github.io/canvas-datagrid/tutorials/demo.html}</li></ul>
 * If you create the grid using the non-web component model you can attach the grid to an existing canvas by passing the canvas in as the `parentNode` when you instantiate the grid.
 * With the exception of attaching to an existing canvas, the grid will attempt to create a Shadow DOM element and attach a canvas within.<br><br>
 * Although the grid itself is pure canvas, there are some supporting elements of the grid that are HTMLElements.<br>
 * <ul><li>In order to overlap parts of the page that are not on the canvas, the context menu is a `&lt;div&gt;`.</li>
 * <li>In order to support advanced key commands, copying, pasting as well as user customization outside of this library, the editing input/textarea is DOM.</li>
 * <li>A hidden input is placed outside of the visible area of the web page to capture keystroke and translate them into grid commands, such as cursor navigation.</li></ul>
<<<<<<< HEAD
 * <br>You can customize almost every behavior of the grid by subscribing to an event the event handler collection.  Most events have `e.preventDefault();` that will allow you to manipulate the grid's behavior on a very granular level.
=======
 * <br>You can customize almost every behavior of the grid by subscribing to an event in the event handler collection.  Most events have `e.preventDefault();` that will allow you to manipulate the grid's behavior on a very granular level.
>>>>>>> master
 * Read the extensive tutorials section or view a demo and check the links in the "see other" section below.<br>
 * This documentation is automatically generated and can be prone to omissions, bad links and errors,
 * Please report any problems with the documentation here {@link https://github.com/TonyGermaneri/canvas-datagrid/issues}.
 * @class
 * @name canvasDatagrid
 ...
 * @param {boolean} [args.touchReleaseAcceleration=90] - How many times the detected pixel per inch touch swipe is multiplied on release.  Higher values mean more greater touch release movement.
