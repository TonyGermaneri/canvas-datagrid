/**
 * Canvas Datagrid is a hierarchical canvas based web component.  The grid can be created in a number of ways.
 * You can use the grid as a web component by using the tag `<canvas-datagrid></canvas-datagrid>` anywhere in your document.
 * You can create the web component by running `var foo = document.createElement('canvas-datagrid')`.
 * You can use the Webpack3 universal module loader by adding one of many module loaders to your application code.
 * Here is an example using AMD module loader {@link https://tonygermaneri.github.io/canvas-datagrid/tutorials/amdDemo.html}.
 * You can also load the grid by invoking the global method `var foo = canvasDatagrid(<args>);`
 ...
 * In order to capture keystrokes, a hidden input is placed outside of the visible area of the web page to capture keystroke and translate them into grid commands, such as cursor navigation.
 ...
 * Read the extensive tutorials section or view a demo and check out the source code below.
 * @class
 * @name canvasDatagrid
