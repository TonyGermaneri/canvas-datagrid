 ...
 * @param {boolean} [args.treeHorizontalScroll=false] - Reserved. Intended to make expanded child grids scroll horizontally with the parent columns; the option is accepted but not implemented yet (see issue #314), child grids always stay stationary.
 ...
 * @param {boolean} [args.allowColumnSelection=true] - When false, clicking or touching a column header never selects (highlights) the column, so with `columnHeaderClickBehavior: 'sort'` an existing row selection survives sorting. Columns can still be selected through the API.
 ...
 * @param {number} [args.maxCanvasSize=16384] - Largest width or height, in CSS pixels, an auto-sized grid (`style.width`/`style.height` set to `auto`) may grow to. Browsers cannot paint larger canvases. Past this size the grid stops growing and shows a scroll bar for the rest of the data.
...
// event docs go here
/**
 * Fires when the active cell changes, e.g. by clicking a cell or moving with the keyboard.
 * @event
 * @name canvasDatagrid#activecellchanged
 * @param {object} e Event object.
 * @param {object} e.cell The new active cell as `{rowIndex, columnIndex}`.
 */
/**
 ...
 */
/**
 * Fires when the data changes: when the data setter is set and after every
 * user or API mutation of the underlying data.
 * @event
 * @name canvasDatagrid#datachanged
 * @param {object} e Event object.
 * @param {object} e.ctx Canvas context.
 * @param {object} e.data The bound (unfiltered, unsorted) data.
 * @param {string} e.source What changed the data. One of `data` (the setter), `edit`, `paste`, `cut`, `delete`, `fill`, `addrow`, `insertrow`, `deleterow`, `addcolumn`, `insertcolumn`, `deletecolumn`.
 * @param {array} [e.cells] For `paste`, `cut` and `delete`: the affected cells as `[rowIndex, columnIndex, boundRowIndex, boundColumnIndex(, value)]` tuples.
 * @param {object} [e.cell] For `edit`: the edited cell.
 * @param {*} [e.value] For `edit`: the new value.
 */
/**
 ...
 * @param {object} e Event object
 * @param {object} e.ctx Canvas context.
 * @param {array} e.cells Cells affected by the paste action. Each item in the array is a tuple of [rowIndex, columnIndex, boundRowIndex, boundColumnIndex, value], where `value` is the pasted value.
 */
/**
