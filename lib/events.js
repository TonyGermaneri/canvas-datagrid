...
export default function (self) {
  ...
  self.pasteData = function (
    ...
    const isSupportedHtml = /(?:^(<meta[^>]+>)?[\s\S]+<div.*?>)|(?:^(<meta[^>]+>)?[\s\S]+<span.*?>)|(?:^(<meta[^>]+>)?<table.*?>)|(?:<!--StartFragment-->\s*<table.*?>)|(?:<!--StartFragment-->[\s\S]+<tr.*?)|(?:<!--StartFragment-->[\s\S]+<td.*?)/.test(
      pasteValue,
    );

    if (mimeType === 'text/html' && !isSupportedHtml) {
      console.warn(
    ...
    function sanitizeElementData(element) {
      let elementData =
        element.nodeType === 1 ? element.innerText : element.data;
      let sanitizedElementData = String(elementData)
        .replace(/\s{2,}/g, ' ')
        .replace(/^\s+/g, '')
        .replace(/\s+$/g, '');
      return sanitizedElementData;
    }

    function parseData(data, mimeType) {
      const isHtmlTable = /(?:<table.*?>)|(?:<tr.*?>)/.test(data);

      if (mimeType === 'text/html' && isHtmlTable) {
        ...
        trs.forEach((tr) => {
          ...
          tr.childNodes.forEach((td) => {
            if (['#text', '#comment'].includes(td.nodeName)) return;
            const col = {
              value: [],
            };
            let value = sanitizeElementData(td);
            if (value)
              col.value.push({
                value: value,
              });
