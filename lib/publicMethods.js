...
export default function (self) {
  ...
  self.isMultiColumnsSelected = function (columnIndex) {
    ...
    self.viewData.forEach(function (row, rowIndex) {
      var columnIndics = self.selections[rowIndex];
      if (
        !columnIndics ||
        columnIndics.length <= 1 ||
        columnIndics.indexOf(columnIndex) === -1
      ) {
        multiColIsSelected = false;
      } else if (columnIndics.length > 1) {
        if (columnIndics[0] != columnIndex) multiColIsSelected = false;
        else {
          for (var i = 0; i < columnIndics.length - 1; i++) {
            if (columnIndics[i] + 1 != columnIndics[i + 1]) {
              multiColIsSelected = false;
              break;
            }
          }
