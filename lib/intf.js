...
export default function (self, ctor) {
  ...
  self.parseFont = function (fontStyle) {
    ...
    var m = /(\d*\.?\d+)(px|pt|em|rem|%)?(?:\s*\/\s*[\w.%]+)?\s+(.+)$/.exec(
      value,
    );
    if (!m) {
      return {
  ...
  self.getScaledFont = function (styleKey) {
    ...
    return (
      (prefix ? prefix + ' ' : '') +
      self.style[styleKey + 'Height'] * self.scale +
      'px ' +
      self.style[styleKey + 'Name']
    );
  };
  ...
  self.etl = function (data, callback) {
    ...
    self.intf.parsers[self.dataType](data, function (data, schema) {
      ...
      if (self.getSchema()) {
        ...
        if (
          !self.orders.columns ||
          self.orders.columns.length !== columnCount
        ) {
          self.createColumnOrders();
        }
      }
