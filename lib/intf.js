...
export default function (self, ctor) {
  ...
  self.toggleCollapseTree = function (rowIndex, columnIndex, type) {
    ...
    if (
      columnIndex == self.celltree.rowsColIndex &&
    ...
    for (let k in self.celltree.origin.rows) {
      ...
      if (!tree.hide) {
        ...
        if (k < self.celltree.columnTreeRowIndex) {
          tempData.push(self.originalData[k]);
        } else {
          for (let l in self.originalData[k]) {
            if (!self.celltree.tempSchema[parseInt(l)].hidden) {
    ...
    if (collapsedCount) {
      let blankRow = Array(self.viewData[0].length).fill('');
      ...
      tempData.push(...Array(collapsedCount).fill(blankRow));
    }
  ...
  self.initCellTreeSettings = function () {
    ...
    if (self.attributes.rowTree.length > 0 && self.viewData.length > 0) {
      ...
      self.celltree.rowsColIndex = self.attributes.rowTreeColIndex;
    ...
    if (self.attributes.columnTree.length > 0 && self.viewData.length > 0) {
      self.celltree.columnTreeRowIndex = self.attributes.columnTreeRowIndex;
