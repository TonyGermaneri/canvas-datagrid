...
export default function (self, ctor) {
  ...
  self.toggleGroup = function (group) {
    if (group.type === 'c') {
      ...
      const schema = self.getSchema();
      for (
        let columnIndex = matchedGroup.from;
        columnIndex <= matchedGroup.to;
        columnIndex++
      )
  ...
  self.hasCollpasedRowGroup = function () {
  ...
  self.getFilteredAndSortedViewData = function (originalData) {
    ...
    //#region Hide rows from collpased group
    /** @type {number[][]} */
    let collapsedGroups = [];
    for (let i = 0; i < self.groupedRows.length; i++) {
    ...
    //#endregion Hide rows from collpased group
