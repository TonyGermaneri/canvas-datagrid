...
export default function (self, ctor) {
   ...
   * - `collpased`: Is this group be collapsed
  ...
  self.getColumnGroupAreaHeight = function () {
    ...
    const base = (self.sizes.rows[-1] || self.style.columnHeaderCellHeight) * self.scale;
    return base * groups;
  }
  /**
  ...
  self.getRowGroupAreaWidth = function () {
    ...
    const base = (self.sizes.columns[-1] || self.style.rowHeaderCellWidth) * self.scale;
    return base * groups;
  }
  /**
   * Toggle the collapse status of a group (expanded/collapsed)
   * @param {{type:'c'|'r',from:number;to:number}} group
   */
  self.toggleGroup = function (group) {
    if (group.type === 'c') {
      ...
      const schema = self.getSchema();
      for (let columnIndex = matchedGroup.from; columnIndex <= matchedGroup.to; columnIndex++)
        schema[columnIndex].hidden = nextCollapsed;
