...
export default function (self) {
  ...
  self.resize = function (drawAfterResize) {
    ...
    function checkScrollBoxVisibility() {
      ...
      self.scrollBox.horizontalBarVisible =
        ((self.style.width !== 'auto' || capped.width) &&
      ...
      self.scrollBox.verticalBarVisible =
        ((self.style.height !== 'auto' || capped.height) &&
    ...
    function setCanvasSize() {
      ...
      self.autoSizeCapped = { width: false, height: false };
  ...
  self.click = function (e, overridePos) {
    ...
    if (
      ...
      (self.currentCell.context === 'cell' ||
        self.currentCell.dragContext === 'column-reorder')
    ) {
  ...
  self.dragResizeColumn = function (e) {
    ...
    if (
      resizingColumn
        ? self.dispatchEvent('resizecolumn', resizeEventDetails)
        : self.dispatchResizeRow(resizeEventDetails)
    ) {
      return false;
    }
  ...
  self.mousedown = function (e, overridePos) {
    ...
    if (['row-reorder', 'column-reorder'].indexOf(self.dragMode) !== -1) {
      ...
      if (
        self.dragMode === 'column-reorder' &&
        !self.isMultiColumnsSelected &&
        self.attributes.allowColumnSelection
      ) {
        self.selectColumn(self.currentCell.header.index, ctrl, shift);
  ...
  self.pasteData = function (
    ...
    self.dispatchDataChanged('paste', { cells: affectedCells });
   ...
   */
  self.expandGridForInsert = function (
    rows,
    requiredRowCount,
    requiredColumnCount,
  ) {
    ...
    if (missingColumns > 0) {
      ...
      for (let i = 0; i < missingColumns; i += 1) {
        const column = {
          ...
          type: deduceColTypeFromData(rows, i),
          title: ' ',
  ...
  self.insert = function ({
    ...
    if (self.attributes.allowGridExpandOnPaste) {
      self.expandGridForInsert(
        rows,
        startRowIndex + rowsLength,
        startColumnIndex +
          Math.max(minColumnsLength, ...rows.map((cells) => cells.length)),
      );
    }
  ...
  self.cut = function (event) {
    ...
    self.dispatchDataChanged('cut', { cells: apiCompatibleCells });
