...
export default function (self) {
  ...
  self.resize = function (drawAfterResize) {
    ...
    v.height =
      self.height - columnHeaderCellHeight - topGroupAreaHeight - self.style.scrollBarWidth - d - m;
    // vertical box
  ...
  self.click = function (e, overridePos) {
    ...
    const columnGroup = self.getColumnGroupAt(pos.x, pos.y)
    if (columnGroup) {
      if (self.toggleGroup(columnGroup)) {
        self.setStorageData();
        setTimeout(function () {
          self.resize(true);
        }, 10);
