...
Version note: the project restarted its numbering at 0.3.0 in 2021 while the
registry still held 0.22 to 0.25.4, so every release since then sorted below
0.25.4 and `latest` had to be forced by hand. This release continues from
0.25.4 instead, which makes npm's version ordering correct again. There is
no 0.5.0 release; the changes below were briefly tagged as such.
