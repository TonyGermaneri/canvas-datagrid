        ...
        if (row) {
          ...
          sSorted.forEach(function (column, columnIndex) {
            if (rowKeys.indexOf(column.name) !== -1) {
              ...
              // escape strings
              addCellValue(row[column.name], trRow, textRow);
            }
          });
...
function contextMenu (self) {
  ...
  function createContextMenu(ev, pos, items, parentContextMenu) {
    ...
    function init() {
      ...
      downArrow.addEventListener('mouseout', endHoverScroll());
      upArrow.addEventListener('mouseover', startHoverScroll('up'));
      upArrow.addEventListener('mouseout', endHoverScroll());
      checkArrowVisibility();
    }
...
// export global
if (window && !window.canvasDatagrid && !true) {
