...
define([], function context() {
    ...
    function grid(args) {
        ...
        var defaultAttributes = [
            ...
            activeCell = [0, 0],
            currentObject,
            storageName = 'canvasGrid',
            storedSettings,
            invalidSearchExpClass = 'canvas-grid-invalid-search-regExp',
            uniqueId = 'canvasGridUniqueId',
            orderBy = uniqueId,
            orderDirection = 'asc',
            invalidFilterRegEx,
            filterBy = '',
        ...
        function getSelectedData() {
            ...
            selections.forEach(function (row, index) {
                ...
                row.forEach(function (col) {
                    d[index][col] = data[index][s[col].name];
                });
            });
        }
        function selectArea() {
        ...
        function draw() {
            ...
            var t, rowHeaderCell, p, cx, cy, cellHeight, cornerCell, y, x, c, h, w, s, end;
                ...
                return function drawEach(header, headerIndex) {
                    ...
                    if (!fire('rendercell', [ctx, d[header.name], d, header, data, schema, cx, cy], intf)) {
                        ...
                        if ((attributes.showRowNumbers && cellStyle === 'headerRowCell')
                            ...
                            if (val === undefined && !f) {
                                val = '';
                                console.warn('canvas-grid: I don\'t know how to format a '
                            ...
                            ctx.fillText(addEllipsis(val, cellWidth - style[cellStyle + 'PaddingRight'] - orderByArrowSize),
                                ...
                                cy - t.alphabeticBaseline - style[cellStyle + 'PaddingBottom']);
                            ctx.restore();
                        }
                        x += cellWidth;
                    }
            ...
            data.forEach(function (d, index) {
                ...
                if (attributes.showRowHeaders) {
                    ...
                    rowHeaderCell[uniqueId] = d[uniqueId];
                    a = {
                        name: 'headerRowCell',
        ...
        function mousemove(e) {
            ...
            if (fire('mousemove', [e, o && o.item && o.item.data], intf)) { return; }
            draw();
        }
        ...
        function setFilter(column, value) {
            ...
            filterBy = column;
            filterValue = value;
            header = (schema || tempSchema).filter(function (col) {
        ...
        function contextmenu(e) {
            ...
            filterLabel.className = 'canvas-grid-context-menu-label';
            filterInput = document.createElement('input');
            filterLabel.innerHTML = 'Filter';
            filterContainer.appendChild(filterLabel);
            filterContainer.appendChild(filterInput);
            contextMenu = document.createElement('div');
            contextMenu.className = 'canvas-grid-context-menu';
            ...
            if (fire('contextmenu', [contextObject, menuItems, contextMenu], intf)) { return; }
            if (!menuItems.length) {
                return;
            }
            menuItems.forEach(function (item) {
                var row = document.createElement('div');
                row.className = 'canvas-grid-context-menu-item';
        ...
        function beginEditAt(x, y) {
            ...
            function postDraw() {
                ...
                textarea.className = 'canvas-grid-edit-textarea';
                textarea.style.position = 'absolute';
                textarea.style.border = 'none';
                textarea.style.top = top + 'px';
                textarea.style.left = left + 'px';
                //TODO what the fuck is this * 6 shit?  I don't know how this works.
                textarea.style.height = cell.height - (style.cellBorderWidth * 6) + 'px';
                textarea.style.width = cell.width - (style.cellBorderWidth * 6) + 'px';
        ...
        function attachCss() {
            var styleSheet, css = [
                '.canvas-grid-edit-textarea { padding: 2px; font-size: '
                    + style.editCellFontSize + '; font-family: '
                    + style.editCellFontFamily + '; }',
                '.canvas-grid-context-menu-item { margin: 2px; }',
                '.canvas-grid-context-menu-label { display: inline-block; min-width: 75px; }',
                '.canvas-grid-context-menu { font-family: sans-serif; background: #CCC; border: 1px solid #555; padding: 2px; border-radius: 3px; opacity: 0.97;}',
                '.canvas-grid-invalid-search-regExp { background: #E99; }'
            ].join('\n');
            ...
            styleSheet.href = 'data:text/css;base64,' + btoa(args.css || css);
        }
        function scroll() {
            draw();
            if (textarea) {
